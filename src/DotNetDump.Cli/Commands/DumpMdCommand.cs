...
/// <summary>
/// <c>dndump dumpmd &lt;address&gt;</c> -- CLI_DESIGN.md §4.5. Display MethodDesc details.
/// </summary>
public static class DumpMdCommand {
	public static Command Create() {
		var addressArgument = new Argument<string>("address") {
			Description = "MethodDesc address (hex, with or without 0x prefix).",
		};

		var command = new Command("dumpmd", "Display MethodDesc details.");
		...
		command.SetAction((ParseResult parseResult) => {
			...
			var info = analyzer.GetMethodDesc(address);
